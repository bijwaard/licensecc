...
BOOST_AUTO_TEST_CASE(set_and_compare_data2) {
	array<uint8_t, HW_IDENTIFIER_PROPRIETARY_DATA_EXT> data = {0xFF, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x50,0x51,0x52,0x53,0x54,0x55};
	HwIdentifier2 pc_id;
	pc_id.set_data(data);
	data[0] = data[0] & 0x1f;
	BOOST_CHECK_MESSAGE(pc_id.data_match(data), "Data match");
...
BOOST_AUTO_TEST_CASE(compare_wrong_data2) {
	array<uint8_t, HW_IDENTIFIER_PROPRIETARY_DATA_EXT> data = {0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x50,0x51,0x52,0x53,0x54,0x55};
	HwIdentifier2 pc_id;
	pc_id.set_data(data);
	data[4] = 0;
	BOOST_CHECK_MESSAGE(!pc_id.data_match(data), "Data shouldn't match");
